...
def line_scatter(x, y_scatter, y_lines, strx='x', stry='y'):
    ...
    ax.plot(y_scatter, '^-', ms=5, lw=2, alpha=0.7, color='black')
    ax.plot(y_lines, 'o-', ms=5, lw=2, alpha=0.7, color='red')
    plt.xlabel(strx)
    plt.ylabel(stry)
    plt.legend()
    plt.show()
...
if __name__ == "__main__":
    ...
    # import
    com_data = pd.read_excel(r'dg.xlsx',
    ...
    y0 = t_y[0].detach().numpy()

    x = list(range(predict.shape[1]))

    line_scatter(x, predict, predict, strx='x', stry='y')
