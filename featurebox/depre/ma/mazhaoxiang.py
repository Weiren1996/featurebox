...
from featurebox.tools.tool import check_random_state, parallize, time_this_function
...
def sympy_prim_set(rep_name,
    ...
    functions1 = {"sin": sympy.sin, 'cos': sympy.cos, 'exp': sympy.exp, 'log': sympy.ln,
                  ...
                  'Zeroo': zeroo, "Oneo": oneo, "Rem": rem, "Self": self, "erf":sympy.erf}
...
def calculate(expr01, pset, x, y, score_method=explained_variance_score, add_coeff=True, filter_warning=True,
    ...
    if add_coeff:
        expr01, a_list = add_coefficient(expr01, **kargs)
        try:
            func0 = sympy.utilities.lambdify([_.value for _ in terminals] + a_list, expr01,modules=["numpy",{"erf":erf}])
        ...
        pass
    try:
        func0 = sympy.utilities.lambdify([_.value for _ in terminals], expr01,modules=["numpy",{"erf":erf}])
        re = func0(*x.T)
        assert_all_finite(re)
        check_array(re, ensure_2d=False)
        ...
        def cv_expr(expr_):
            func0_ = sympy.utilities.lambdify([_.value for _ in terminals], expr_,modules=["numpy",{"erf":erf}])
            ss = ShuffleSplit(n_splits=5, test_size=0.20, random_state=10)
            score_ = []
            for train_index, _ in ss.split(x, y):
                x_train = x[train_index]
        ...
        if del_no_important and isinstance(expr01, sympy.Add) and len(expr01.args) >= 3:
            ...
            for expri in expr01.args:
                if not isinstance(expri, sympy.Float):
                    func0 = sympy.utilities.lambdify([_.value for _ in terminals], expri,modules=["numpy",{"erf":erf}])
                    re = np.mean(func0(*x.T))
                    if abs(re) > abs(0.001 * np.mean(y)):
                        re_list.append(expri)
                else:
...
if __name__ == "__main__":
    ...
    result = main_part(x, y, pset1, pop_n=500, random_seed=2, cxpb=0.8, mutpb=0.2, ngen=3, max_=2,
                       inter_add=True, iner_add=True, random_add=True)